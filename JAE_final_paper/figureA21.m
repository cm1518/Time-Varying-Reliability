%% Figure 1, plots  fixed coefficient and TVP IRFs either all 4 variables, UK version
...
data_file = '4. Data/uk_data_combined.txt'; % this is important, load data in a very specific way
...
% Which TVP-VAR?
VAR_select='ctv_cm2_v1_q_hcauchy';
[priors,options_,q_s0_options]=uk_model_specs_data_combined(VAR_select);
options_.savefig=1;
% clearing objects
sample_iv_init=[];
sample_iv_end=[];
...
folder2 = [currentFolder '/../2. Codes/Main/results/ctv_cm2_v1_fixed'];
...
proxy_name{i}='Cesa-Bianchi et al';

%macrovarSelec=options_.macrovarSelec;
%macro_var_str_names=options_.macro_var_str_names;
macro_var_str_names =  {'One-Year Rate','CPI','Unemployment','Exchange Rate','Corporate Spread','Mortgage Spread','BAA Corporate Spread - US'}; % Name of variables (for plots)
macrovarSelec=[1:7];
...
color_fix   = [.7 .7 .7]+.1;
color_tvp   = [250,128,114]/255;%[ 30,144,255]./255;
...
%     plot(0:1:H,squeeze(SVAR_TVP.LtildeFull(macrovarSelec(ii),1:Horizon,4,1)),'b--','LineWidth',linW)
    ...
    axis([0 H ylim])
    if ii==7,legend([p_1 p_2],{'\beta: Constant','\beta_t: Random Walk'},'Location','NorthEast'),legend boxoff,end
...
set(gcf,'PaperPosition',[.25 .25 18 24])
