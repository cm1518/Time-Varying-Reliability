...
% Which TVP-VAR?
VAR_select='gk_ff4_v1_q_0_0001_r_hcauchy';
...
for ii = 1:length(macrovarSelec) % Variable
    ...
    % Add dashed lines for the boundaries of the fixed-coefficient IRF  
    plot(0:1:H, squeeze(SVAR_Fixed.LtildeFull(macrovarSelec(ii),1:Horizon,2,1)), '--', 'Color', color_fix, 'LineWidth', linW)  
    plot(0:1:H, squeeze(SVAR_Fixed.LtildeFull(macrovarSelec(ii),1:Horizon,4,1)), '--', 'Color', color_fix, 'LineWidth', linW)  
    ...
    % Add dashed lines for the boundaries of the TVP IRF  
    plot(0:1:H, squeeze(SVAR_TVP.LtildeFull(macrovarSelec(ii),1:Horizon,2,1)), '--', 'Color', color_tvp, 'LineWidth', linW)  
    plot(0:1:H, squeeze(SVAR_TVP.LtildeFull(macrovarSelec(ii),1:Horizon,4,1)), '--', 'Color', color_tvp, 'LineWidth', linW)  
    ...
    axis([0 H ylim])
    if ii==1,legend([p_1 p_2],{'log(\sigma_v^2): Constant','log(\sigma_{v,t}^2): Random Walk'},'Location','NorthEast'),legend boxoff,end
