%% Figure 8, plots instrument set to zero, 1 by 3 subplot with CPI IRF (and original fixed coefficient IRF), as well as estimated beta path and reliability
%% setting proxy to zero, version a): proxy to zero if PM0 > 0.5, b): proxy to zero if PM0 <= 0.5
...
load([folder1 '/irf_proxy2zero_v2.mat'])
load([folder1 '/post_proxy2zero_v2.mat'])
f = 0.25/SVAR.LtildeFull(1,1,3);
SVAR.LtildeFull=f*SVAR.LtildeFull;
SVAR_TVPb = SVAR;
...
patch_tvp = [squeeze(SVAR_TVPa.LtildeFull(macrovarSelec,1:Horizon,2,1))';
    flipdim(squeeze(SVAR_TVPa.LtildeFull(macrovarSelec,1:Horizon,4,1)),2)']';
p_2 = patch(patch_axis,patch_tvp,color_tvp,'EdgeColor','None');
plot(0:1:H,squeeze(SVAR_TVPa.LtildeFull(macrovarSelec,1:Horizon,3,1)),...
...
datechar=nDate(sample_iv_init_row(1,1)+pp:sample_iv_end_row(1,1),1);
...
% TVP
patch_tvp = [SVAR_TVPa.BETFull{1,1}(2,:) flipdim(SVAR_TVPa.BETFull{1,1}(4,:),2)];
p_2 = patch(patch_axis,patch_tvp,color_tvp,'EdgeColor','None');
plot(datechar,SVAR_TVPa.BETFull{1,1}(3,:)','Color',color_tvp,'LineStyle','-','LineWidth',linW);
alpha(.5)
...
% TVP
patch_tvp = [SVAR_TVPa.RELFull{1,1}(2,:) flipdim(SVAR_TVPa.RELFull{1,1}(4,:),2)];
p_2 = patch(patch_axis,patch_tvp,color_tvp-.1,'EdgeColor','None');
plot(datechar,SVAR_TVPa.RELFull{1,1}(3,:)','Color',color_tvp,'LineStyle','-','LineWidth',linW);
alpha(.5)
% legend([p_1 p_2],{'Time-Constant','Random Walk'},'Location','NorthEast'),legend boxoff
dateFormat = 10;
datetick('x',dateFormat)
axis tight
set(gca,'YLim',[0 1],'YTick',[0:.2:1],'YTickLabel',{'0','0.2','0.4','0.6','0.8','1'})
grid('on'),box('on')
...
plot(0:1:H,squeeze(SVAR_Fixed.LtildeFull(macrovarSelec,1:Horizon,3,1)),...
    'Color',color_fix-.2,'LineStyle','-','Marker','none','LineWidth',linW)
% plotting tvp coeff IRF
patch_tvp = [squeeze(SVAR_TVPb.LtildeFull(macrovarSelec,1:Horizon,2,1))';
    flipdim(squeeze(SVAR_TVPb.LtildeFull(macrovarSelec,1:Horizon,4,1)),2)']';
p_2 = patch(patch_axis,patch_tvp,color_tvp,'EdgeColor','None');
plot(0:1:H,squeeze(SVAR_TVPb.LtildeFull(macrovarSelec,1:Horizon,3,1)),...
    'Color',color_tvp,'LineStyle','-','Marker','none','LineWidth',linW)
hline(0,'-k')
alpha(.5)
...
plot(datechar,repmat(SVAR_Fixed.BETFull(1,3),1,length(datechar)),'Color',color_fix-.2,'LineStyle','-','LineWidth',linW);
% TVP
patch_tvp = [SVAR_TVPb.BETFull{1,1}(2,:) flipdim(SVAR_TVPb.BETFull{1,1}(4,:),2)];
p_2 = patch(patch_axis,patch_tvp,color_tvp,'EdgeColor','None');
plot(datechar,SVAR_TVPb.BETFull{1,1}(3,:)','Color',color_tvp,'LineStyle','-','LineWidth',linW);
alpha(.5)
% legend([p_1 p_2],{'Time-Constant','Random Walk'},'Location','NorthEast'),legend boxoff
xlabel('Time')
dateFormat = 10;
...
plot(datechar,repmat(SVAR_Fixed.RELFull(1,3),1,length(datechar)),'Color',color_fix,'LineStyle','-','LineWidth',linW);
% TVP
patch_tvp = [SVAR_TVPb.RELFull{1,1}(2,:) flipdim(SVAR_TVPb.RELFull{1,1}(4,:),2)];
p_2 = patch(patch_axis,patch_tvp,color_tvp-.1,'EdgeColor','None');
plot(datechar,SVAR_TVPb.RELFull{1,1}(3,:)','Color',color_tvp,'LineStyle','-','LineWidth',linW);
alpha(.5)
% legend([p_1 p_2],{'Time-Constant','Random Walk'},'Location','NorthEast'),legend boxoff
dateFormat = 10;
datetick('x',dateFormat)
