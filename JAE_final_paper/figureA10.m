...
%     plot(0:1:H,squeeze(SVAR_TVP.LtildeFull(macrovarSelec(ii),1:Horizon,4,1)),'b--','LineWidth',linW)
    ...
    plot(0:1:H,squeeze(SVAR_TVP.LtildeFull(macrovarSelec(ii),1:Horizon,3,1)),'Color',color_ms,'LineStyle','-','LineWidth',linW)
    hline(0,'-k')
    ...
    axis([0 H ylim])
    if ii==4,legend([p_1 p_2],{'\beta: Constant','\beta_t: Markov switching'},'Location','NorthEast'),legend boxoff,end
