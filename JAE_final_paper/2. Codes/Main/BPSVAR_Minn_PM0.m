function [SVAR, draws]=BPSVAR_Minn_PM0(data,proxy,sample_vector,options_,priors)
...
%% prior simulator for SDDR
nrep_prior=10000;

for j=1:nrep_prior
    for i=1:nIV      
        SDDR_denom{i,1}(j,:)=exp(-(1/2)*log(2*pi)- (1/2)*log(V0) - (1/2)*(mu0^2/V0));
    end
...
while record<nd
    ...
    for i=1:nIV
        ...
        numer{i,1} = exp(-(1/2)*log(2*pi)- (1/2)*log(Vp) - (1/2)*(bet(i,1)^2/Vp));
    end
...
SVAR.BFull = quantile(B,ptileVEC);
