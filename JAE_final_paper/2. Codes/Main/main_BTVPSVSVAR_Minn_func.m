function [SVAR, draws]=main_BTVPSVSVAR_Minn_func(data,proxy,sample_vector,options_,priors,q_s0_options,r_s0_options)
...
% beta(t) = beta(t-1) + v(t)
% v(t) ~ N(0,q)
% log(signu(t)^2) = log(signu(t-1)^2) + u(t)
% u(t) ~ N(0,r)
%
% priors
% p(beta(0)) ~ N(b_0_prmean,b_0_prvar)
% p(log(signu(0)^2)) ~ N(lsig_0_prmean,lsig_0_prvar)
% p(q) ~ IG(q_nu0/2, q_nu0*q_s0^2/2)
% p(r) ~ IG(r_nu0/2, r_nu0*r_s0^2/2)
%
...
r_s0 = priors.r_s0; % inverse gamma prior location of the time-variation innovation STANDARD DEVIATION
r_nu0 =priors.r_nu0; % inverse gamma prior degree of freedom 
...
for i=1:nIV
    ...
    Sigtdraw{i,1} = zeros(MM_nT(i,1),1);
    signu{i,1}=exp(.5*Sigtdraw{i,1});
end
...
lnp0 = loglik_tvmsv_given_y_unbalanced(MM, Udraw(ndum+1:end, :),sample_vector, LC, Q(:,1), bet, signu);
...
while record<nd
    ...
    if (rand()<rwmh_sigma_prob)     % draw sigma* ~ IW(sigma, nu), phi*|sigma*, Y
        ...
        lnp1 = loglik_tvmsv_given_y_unbalanced(MM, Udrawstar(ndum+1:end, :),sample_vector, LCstar, Q(:,1), bet, signu);
    ...
    lnp1 = loglik_tvmsv_given_y_unbalanced(MM, Udraw(ndum+1:end, :),sample_vector, LC, Qstar(:,1), bet, signu);
    ...
    for i = 1:nIV
        ...
        [btdraw,log_lik] = carter_kohn(MM{i,1}',Xe(sample_vector(i,1):sample_vector(i,2),1),signu{i,1}.^2,q(i),1,1,MM_nT(i,1),b_0_prmean(i),b_0_prvar(i));   
...
%            q(i)       = inv(Qinvdraw);            % this is a draw from Q
         ...
         if q_s0_options.est_q_s0_flag == 1
             ...
             if  record>0 && record/nadjust == round(record/nadjust) && record<=bburn && q_s0_options.acc_tuning==1 %% Tuning phase     
                [q_s0_options.tuning_para(i)]=automatic_stabilization(q_s0_options.tuning_para(i),qjsux(i),qjrep(i),q_s0_options);
                qjsux(i)=0;qjrep(i) = 0;
             end

            qjsux(i)=qjsux(i)+accept_flag;
            qjrep(i) = qjrep(i) + 1;
        
         end
         
                  
       %% Draw stochastic volatilities sig(t)
        %------------------------------------------------------------ 
        ...
        yhat = zeros(1,MM_nT(i,1));
        Ztmp=MM{i,1};
        Xtmp=Xe(sample_vector(i,1):sample_vector(i,2),1);
        for ii = 1:MM_nT(i,1)
            yhat(1,ii) = Ztmp(ii,1) - Xtmp(ii,1)*btdraw(ii,1) ;
        end
...
%         end
        ...
        [Sigtdraw{i,1}(:,1) , statedraw] = SVRW2(yss,Sigtdraw{i,1},r(i),sigma_prmean(i),sigma_prvar(i),1);

        signu{i,1} = exp(.5*Sigtdraw{i,1}); % standard deviations

        e2 = Sigtdraw{i,1}(2:end,1) - Sigtdraw{i,1}(1:end-1,1);   
        r_nu1 = MM_nT(i,1) + r_nu0(i); 
        r_s12 = sum(e2.^2)' + r_nu0(i)*r_s0(i)^2;
        r(i) = inverse_gamma(1,r_nu1/2, r_s12/2);

        %%  estimate hyperparameter r_s0, draw from Random-Walk MH block

        if r_s0_options.est_r_s0_flag == 1
             loglik_old=[];
             loglik_new=[];
             % Get likelihood of r given the current r_s0. This is just the prior of r(given r_s0)
             loglik_old=linv_gam_pdf(r(i),r_nu0(i)/2, r_nu0(i)*r_s0(i)^2/2);
             % prior density for r_s0, depending on the exact prior
             lpriordens_old = priordens(r_s0(i), r_s0_options);
             % posterior kernel of the old draw   
             loglik_old = loglik_old + lpriordens_old; 
             % Draw a new q_s0 from random walk MH
             r_s0_new = r_s0(i)+randn*r_s0_options.tuning_para(i); 
             % Calculate the likelihood of q given the new q_s0
             lpriordens_new = priordens(r_s0_new, r_s0_options); %evaluate prior density with the new kQ draw
             % posterior kernel of the new draw             
             loglik_new = linv_gam_pdf(r(i),r_nu0(i)/2, r_nu0(i)*r_s0_new^2/2);
             loglik_new = loglik_new+lpriordens_new;
             % Calculate the acceptance ratio; 
             logalpha = loglik_new-loglik_old;
             % Accept according to Metropolis algorithm
             if(log(rand)<logalpha) && r_s0_new>r_s0_options.lower_bound && r_s0_new<r_s0_options.upper_bound
                r_s0(i)=r_s0_new;
                accept_flag=1;
                acpt_r(i) = acpt_r(i) + 1;
             else
             ...
             % automatic stabilization of MH
             if  record>0 && record/nadjust == round(record/nadjust) && record<=bburn && r_s0_options.acc_tuning==1 %% Tuning phase     
                [r_s0_options.tuning_para(i)]=automatic_stabilization(r_s0_options.tuning_para(i),rjsux(i),rjrep(i),r_s0_options);
                rjsux(i)=0;rjrep(i) = 0;
             end

            rjsux(i)=rjsux(i)+accept_flag;
            rjrep(i) = rjrep(i) + 1;
        ...
        [alphat,Vt] = durbin_koopman(MM{i,1}',Xe(sample_vector(i,1):sample_vector(i,2),1),signu{i,1}.^2,q(i),1,1,MM_nT(i,1),b_0_prmean(i),b_0_prvar(i),[]);
    ...
    lnp0 = loglik_tvmsv_given_y_unbalanced(MM, Udrawstar(ndum+1:end, :),sample_vector, LCstar, Q(:,1), bet, signu);
    ...
    if record > bburn
        ...
        for i = 1:nIV    
            REL{i,1}(record-bburn,:) = bet{i,1}.^2./(bet{i,1}.^2 + signu{i,1}.^2);
            BET{i,1}(record-bburn,:) = bet{i,1};
            SIG{i,1}(record-bburn,:) = signu{i,1};
            ...
            rdraws(record-bburn,i) =r(i);
            r_s0draws(record-bburn,i) =r_s0(i);
       end
...
for i =1:nIV
    ...
    SVAR.SIGFull{i,1} =  quantile(SIG{i,1},ptileVEC);
end
SVAR.qFull  = quantile(qdraws,ptileVEC);
SVAR.q_s0Full  = quantile(q_s0draws,ptileVEC);
SVAR.rFull  = quantile(rdraws,ptileVEC);
SVAR.r_s0Full  = quantile(r_s0draws,ptileVEC);
