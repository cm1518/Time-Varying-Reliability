function [SVAR, draws]=main_BTVPSVAR_x2lags_Minn_func(data,proxy,sample_vector,options_,priors,q_s0_options)
...
%xreg=lag(YY,1);
%xreg=lag(YY,1)-lag(YY,2);
xreg = [ones(size(YY,1),1) lag(YY,1) lag(YY,2)];
...
%            q(i)       = inv(Qinvdraw);            % this is a draw from Q
        ...
        %a = chol(posterior_covariance)*randn(n+1,1)+posterior_mean; 
