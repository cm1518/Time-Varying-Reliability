function [SVAR, draws]=main_BTVPSVAR_x_Minn_func(data,proxy,sample_vector,options_,priors,q_s0_options)
...
% with lag of data as exogenous regressors in the proxy equation
% The proxy equation is
% m(t) = beta(t)*e_shock(t) + a*data(t-1) + nu(t)
% nu(t) ~ N(0,signu)
...
% We use the scaled inverse Chi-square variant of the IG distribution see
...
nex_ = 1;                                       % number of deterministic variables? one intercept. 
...
ptileVEC = [0.05 0.16 0.50 0.86 0.95]; % Percentiles to be computed
...
b_0_prmean=priors.b_0_prmean;   % initial prior mean of time-varing proxy regression coefficient
b_0_prvar=priors.b_0_prvar;     % initial prior variance of of time-varing proxy regression coefficient
s0 = priors.s0;                 % location parameter for IG (scaled-inverse Chi-square) prior on signu 
nu0 = priors.nu0;               % degree of freedom for IG (scaled-inverse Chi-square) prior on signu
...
Vc=priors.Vc; % prior variance of the intercept.
...
rwmh_sigma_prob = options_.rwmh_sigma_prob;            % probability to draw from the alternative IW distribution 
...
if priors.MP==1  % do we optimize the prior hyperparameter?
    if priors.psitune==0 % covariance of the VAR residuals, tuned to pre-sample variance of observables   
        res = bvarGLP(YY,p,'mcmc',0,'sur',0,'noc',0,'MNalpha',1,'MNpsi',0,'pos',pos);
    elseif priors.psitune==1 % covariance of the VAR residuals, tuned to AR(1) residual estimates of full sample observables  
        res = bvarGLP(YY,p,'mcmc',0,'sur',0,'noc',0,'MNalpha',1,'MNpsi',0,'MNpsitune',1,'pos',pos);
    end
    lambda=res.postmax.lambda; % overall tightness of Minnesota prior
    alpha=res.postmax.alpha; % lag decay of Minnesota prior
elseif priors.MP==0
...
[Trend,Y3] = hpfilter(YY(:,3));
[Trend,Y2] = hpfilter(YY(:,2));
Ydetrend=[YY(:,1) Y2 Y3 YY(:,4)];


xreg = [ones(size(YY,1),1) mlag2(Ydetrend,1)];

xreg = xreg(p+1:end,:);


%a_prmean=priors.a_prmean; % prior mean of coeffcients on lags of data in the proxy equation
%a_prvar=priors.a_prvar; % prior variance of coeffcients on lags of data in the proxy equation
a_prmean = zeros(size(xreg,2),1);
a_prvar = 10*eye(size(xreg,2));
...
lnp0 = loglik_tvm_given_y_unbalanced_x(MM, Udraw(ndum+1:end, :),sample_vector, LC, Q(:,1), bet, signu,xreg,a);
...
while record<nd
    ...
    if (rand()<rwmh_sigma_prob)     % draw sigma* ~ IW(sigma, nu), phi*|sigma*, Y
        ...
        lnp1 = loglik_tvm_given_y_unbalanced_x(MM, Udrawstar(ndum+1:end, :),sample_vector, LCstar, Q(:,1), bet, signu,xreg,a);
    ...
    lnp1 = loglik_tvm_given_y_unbalanced_x(MM, Udraw(ndum+1:end, :),sample_vector, LC, Qstar(:,1), bet, signu,xreg,a);
    ...
    for i = 1:nIV
        ...
        % draw a
        yfit=(MM{i,1}-Xe(sample_vector(i,1):sample_vector(i,2),1).*bet{i,1});
        [posterior_mean, posterior_covariance] = bayesian_linear_regression_posterior(xreg(sample_vector(i,1):sample_vector(i,2),:), yfit, a_prmean, a_prvar, signu(i)^2);
        a = chol(posterior_covariance)*randn(nx,1)+posterior_mean;
       % plot(xreg(sample_vector(i,1):sample_vector(i,2),:)*a)
       % plot(yfit)
        
        % btdraw is a draw of the coefficients, b(t), conditional on
        % all other parameteres

        %[btdraw,log_lik] = carter_kohn_x(MM{i,1}',Xe(sample_vector(i,1):sample_vector(i,2),1),repmat(signu(i)^2,MM_nT(i,1),1),q(i),1,1,MM_nT(i,1),b_0_prmean(i),b_0_prvar(i),xreg(sample_vector(i,1):sample_vector(i,2),:)',a);   
        [btdraw,log_lik] = carter_kohn(MM{i,1}'-a'*xreg(sample_vector(i,1):sample_vector(i,2),:)',Xe(sample_vector(i,1):sample_vector(i,2),1),repmat(signu(i)^2,MM_nT(i,1),1),q(i),1,1,MM_nT(i,1),b_0_prmean(i),b_0_prvar(i));   
...
%            q(i)       = inv(Qinvdraw);            % this is a draw from Q
        ...
        if (prior_type == 'IG')          
            ...
            tmp = (MM{i,1}-Xe(sample_vector(i,1):sample_vector(i,2),1).*bet{i,1} - xreg(sample_vector(i,1):sample_vector(i,2),:)*a);
            s12 = ( tmp'*tmp+ nu0*s0^2);


            %s12 = ( (MM{i,1}-Xe(sample_vector(i,1):sample_vector(i,2),1).*bet{i,1})'*(MM{i,1}-Xe(sample_vector(i,1):sample_vector(i,2),1).*bet{i,1}) + nu0*s0^2);
            %signu(i) = igrand(s1, nu1);
            signu(i) = sqrt(inverse_gamma(1,nu1/2,s12/2));

        elseif (prior_type == 'FI')     
            signu(i) = pr_trunc*std(MM{i,1});
        end


        % % SDDR
        % % DK smoother to get smoothed tv parameter states
        % [alphat,Vt] = durbin_koopman(MM{i,1}',Xe(sample_vector(i,1):sample_vector(i,2),1),repmat(signu(i).^2,MM_nT(i,1),1),q(i),1,1,MM_nT(i,1),b_0_prmean(i),b_0_prvar(i),[]);
        % 
    ...
    lnp0 = loglik_tvm_given_y_unbalanced_x(MM, Udrawstar(ndum+1:end, :),sample_vector, LCstar, Q(:,1), bet, signu,xreg,a);
    ...
    if record > bburn
        ...
        for i = 1:nIV    
            REL{i,1}(record-bburn,:) = bet{i,1}.^2./(bet{i,1}.^2 + signu(i).^2);
...
SVAR.aFull = quantile(adraws,ptileVEC);
