...
savefolder = [currentFolder '/results/MC_v5_1_MSDGP'];
mkdir(savefolder)   
...
parfor i=1:n_sims
    ...
    simulatedStates = simulateMarkovChain(P, initialState, T-options_.p);  
    data=[];
    proxy=[];
    ...
    for t=1:T-options_.p
        if simulatedStates(t)==1
            proxy{1,1}(t,1)=randn*sqrt(kappa)*sigma_m;
        end
    end
