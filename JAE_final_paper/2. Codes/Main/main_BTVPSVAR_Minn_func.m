function [SVAR, draws]=main_BTVPSVAR_Minn_func(data,proxy,sample_vector,options_,priors,q_s0_options)
...
lnp0 = loglik_tvm_given_y_unbalanced(MM, Udraw(ndum+1:end, :),sample_vector, LC, Q(:,1), bet, signu);
...
while record<nd
    ...
    if (rand()<rwmh_sigma_prob)     % draw sigma* ~ IW(sigma, nu), phi*|sigma*, Y
        ...
        lnp1 = loglik_tvm_given_y_unbalanced(MM, Udrawstar(ndum+1:end, :),sample_vector, LCstar, Q(:,1), bet, signu);
    ...
    lnp1 = loglik_tvm_given_y_unbalanced(MM, Udraw(ndum+1:end, :),sample_vector, LC, Qstar(:,1), bet, signu);
    ...
    for i = 1:nIV
        ...
        [btdraw,log_lik] = carter_kohn(MM{i,1}',Xe(sample_vector(i,1):sample_vector(i,2),1),repmat(signu(i)^2,MM_nT(i,1),1),q(i),1,1,MM_nT(i,1),b_0_prmean(i),b_0_prvar(i));   
...
%            q(i)       = inv(Qinvdraw);            % this is a draw from Q
    ...
    lnp0 = loglik_tvm_given_y_unbalanced(MM, Udrawstar(ndum+1:end, :),sample_vector, LCstar, Q(:,1), bet, signu);
