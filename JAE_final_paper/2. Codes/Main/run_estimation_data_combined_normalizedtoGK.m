...
% folders
savefolder = [currentFolder '/results_norm2GK/' VAR_select];
mkdir(savefolder)   
...
%% Proxy normalized to GK's standard deviation
% load ff4_tc
[~,i_ff4_tc] = ismember('ff4_tc',text(1,2:end));
sample_ff4_tc_init = datenum('1991-01-01', 'yyyy-mm-dd');
sample_ff4_tc_end= datenum('2012-06-01', 'yyyy-mm-dd');
[~, sample_ff4_tc_init_row] = ismember(sample_ff4_tc_init,nDate,'rows');
[~, sample_ff4_tc_end_row] = ismember(sample_ff4_tc_end,nDate,'rows');
ff4_tc = YYdata(sample_ff4_tc_init_row:sample_ff4_tc_end_row,i_ff4_tc);

ff4_tc_stdev=std(ff4_tc);

for i=1:nIV

    proxy{i,1} = proxy{i,1}/std(proxy{i,1})*ff4_tc_stdev;
...
[SVAR, draws]=main_BTVPSVAR_Minn_func(data,proxy,sample_vector,options_,priors,q_s0_options);
IRF=SVAR.LtildeFull;
