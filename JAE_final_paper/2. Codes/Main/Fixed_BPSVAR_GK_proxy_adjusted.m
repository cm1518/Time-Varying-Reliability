...
% folders
savefolder = [currentFolder '/results_proxy_adj/' VAR_select];
mkdir(savefolder)   
...
tmp_=mlag2(YYdata(:,i_var),pp);

for i=1:nIV    
    y_=proxy{i,1}(pp+1:end);
    ylag_=mlag2(proxy{i,1},pp);
    X_=[ylag_(pp+1:end,:) tmp_(sample_iv_init_row(i,1)+pp:sample_iv_end_row(i,1),:)];
    b_=(X_'*X_)\(X_'*y_);
    proxy_adj{i,1}=y_-X_*b_;
    sample_vector(i,1) =  sample_vector(i,1)+pp;  
...
SVAR=BPSVAR_Minn_PM0(data,proxy_adj,sample_vector,options_,priors);
IRF=SVAR.LtildeFull;
