function [xi_draw] = carter_kohn_kalman(y,x,A,H,R,F,Q,T,r,xi10,P10)
...
% draw xi_draw(T|T) ~ N(xi(T|T),P(T|T))

xi_draw(:,T) = mvnrnd(xi_tt(:,T)',P_tt(:,:,T));

% Backward recurssions
for t=T-1:-1:1
    J = P_tt(:,:,t)*F(:,:,t)'/P_pred(:,:,t+1);
    cfe = xi_draw(:,t+1) - xi_pred(:,t+1);
    bmean = xi_tt(:,t) + J*cfe;
    bvar = P_tt(:,:,t) - J*F(:,:,t)*P_tt(:,:,t);
    % draw xi_draw(t|t+1) ~ N(xi(t|t+1),P(t|t+1))
    xi_draw(:,t) = mvnrnd(bmean',bvar); 
end
