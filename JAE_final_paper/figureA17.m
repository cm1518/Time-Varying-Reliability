%% Figure 3, plots cleaned instrument 1 by 3 subplot with CPI IRF (and original fixed coefficient IRF), as well as estimated beta path and reliability
...
% Which TVP-VAR?
VAR_select='gk_ff4_v1_fixed';
[priors,options_]=model_specs_data_combined_fixed_coeff(VAR_select);
options_.savefig=1;
% clearing objects
sample_iv_init=[];
sample_iv_end=[];
...
%     plot(0:1:H,squeeze(SVAR_Fixed.LtildeFull(macrovarSelec(ii),1:Horizon,4,1)),'r--','LineWidth',linW)     
    patch_fix = [squeeze(SVAR_Raw.LtildeFull(macrovarSelec(ii),1:Horizon,2,1))';
        flipdim(squeeze(SVAR_Raw.LtildeFull(macrovarSelec(ii),1:Horizon,4,1)),2)']';
    p_1 = patch(patch_axis,patch_fix,color_fix,'EdgeColor','None');
    plot(0:1:H,squeeze(SVAR_Raw.LtildeFull(macrovarSelec(ii),1:Horizon,3,1)),'Color',color_fix-.2,'LineStyle','-','LineWidth',linW)
...
%     plot(0:1:H,squeeze(SVAR_TVP.LtildeFull(macrovarSelec(ii),1:Horizon,4,1)),'b--','LineWidth',linW)
    patch_tvp = [squeeze(SVAR_P20.LtildeFull(macrovarSelec(ii),1:Horizon,2,1))';
        flipdim(squeeze(SVAR_P20.LtildeFull(macrovarSelec(ii),1:Horizon,4,1)),2)']';
    p_2 = patch(patch_axis,patch_tvp,color_tvp,'EdgeColor','None');
    plot(0:1:H,squeeze(SVAR_P20.LtildeFull(macrovarSelec(ii),1:Horizon,3,1)),'Color',color_tvp,'LineStyle','-','LineWidth',linW)
    hline(0,'-k')
    ...
    axis([0 H ylim])
    if ii==4,legend([p_1 p_2],{'Original Instrument','$\overline{m}_t$'},'Location','NorthEast','Interpreter','latex'),legend boxoff,end
...
set(findall(gcf,'-property','FontSize'),'FontSize',11,'FontWeight','Normal','FontName','Times New Roman')
